...
import { protectFragmentForDisplay, neutralizeHtmlForSafeParsing } from './network-policy.js';
...
export function buildV15Html(conv, options={}) {
  ...
  const hasVisual=Boolean(options.visualHtml);
  const visual=hasVisual
    ? `<div class="visual-gate"><strong>Visual Replica</strong><span>自動外部アクセスを防ぐため、必要なときだけ読み込みます。</span><button id="loadVisual">Visual Replicaを表示</button><div id="visualHost"></div></div>`
    : `<div class="empty">Visual Replicaはこの会話に関連付けられたHTMLキャプチャがある場合に表示できます。</div>`;
  const visualSafe=hasVisual?neutralizeHtmlForSafeParsing(options.visualHtml,{preserveStyles:true}):'';
  ...
  return `<!doctype html><html lang="ja" data-ai-archive="v15" data-archive-version="0.4.0"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="referrer" content="no-referrer"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; img-src data: blob:; media-src data: blob:; style-src 'unsafe-inline'; script-src 'unsafe-inline'; frame-src 'self' data: blob:; object-src 'none'; base-uri 'none'; form-action 'none'"><title>${escapeHtml(conv.title)} — Archive</title><style>${v15Css()}</style></head><body>
...
<main><section id="visual" class="tab">${visual}</section><section id="semantic" class="tab active"><div class="meta">Nodes ${data.nodeCount} · Paths ${data.pathCount} · 外部通信は自動禁止</div>${transcript}</section><section id="raw" class="tab"><pre>${escapeHtml(raw)}</pre></section></main>
<script type="application/json" id="archive-conversation">${safeScriptJson(snapshot)}</script>
${hasVisual?`<script type="application/json" id="visual-replica-source">${safeScriptJson(visualSafe)}</script>`:''}
...
function archiveSnapshot(conv){
  ...
  return {format:'AI Dialogue Conversation Snapshot',version:'0.4.0',conversation:{
...
function renderBody(m){return m.richHtml ? protectFragmentForDisplay(m.richHtml) : renderText(m.text);}
